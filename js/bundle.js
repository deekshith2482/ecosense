...
(function() {
  ...
  class AnalyticsDashboard {
    ...
    renderCharts() {
      if (typeof Chart === "undefined") {
        console.warn("⚠️ [EcoSense Analytics] Chart.js library not detected from CDN.");
        return;
      }
      
      // Delay slightly to ensure tab container is fully visible and has layout dimensions
      setTimeout(() => {
    ...
    _renderZoneDistribution() {
      ...
      const incidents = (this.app && this.app.incidents) ? this.app.incidents : [];
  ...
  const bootEcoSense = () => {
    if (!window.ecoApp) {
      window.ecoApp = new EcoSenseApp();
      window.ecoApp.init();
    }
  };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootEcoSense);
  } else {
    bootEcoSense();
  }
})();
